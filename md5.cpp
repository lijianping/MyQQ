 ...
 * @param output Encode data as OUT parameter.
 * @param input Input data.
 * @param len The length of the input assuming it is a multiple of 4
 **/
static void Encode(unsigned char *output, unsigned int *input, unsigned int len);

/*
 * @brief Decodes input data.
 ...
 * @param output OUT parameter where POINTER is an unsigned char *.
 * @param input Data to copy where POINTER is an unsigned char *.
...
void MD5Update(MD5_CTX *context, unsigned char *input, unsigned int input_len) {
	unsigned int i, index, partLen;
	  ...
	  if ( (context->count[0] += ((unsigned long int)input_len << 3))
	       < ((unsigned long int)input_len << 3))
		context->count[1]++;

	  context->count[1] += ((unsigned long int)input_len >> 29);
	  partLen = 64 - index;
...
int MD5(unsigned char *input, unsigned int input_len, unsigned char *digest)
{
	...
	MD5Init(&context);
	MD5Update(&context, input, input_len);
	MD5Final(&context, d);

	for (i = 0; i < 16; ++i) {
		sprintf((char *)digest + i * 2, "%02X", d[i]);
	}
